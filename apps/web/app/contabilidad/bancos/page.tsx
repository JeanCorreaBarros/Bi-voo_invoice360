...
export default function BancosPage() {
  ...
  const emptyForm = { bankName: "", accountNumber: "", accountType: "AHORROS", accountId: "" }
  ...
  const fetchAll = async () => {
    ...
    try {
      const [bankRes, accRes] = await Promise.all([
        fetch(`${apiBase}treasury/bank-accounts`, { headers: authHeaders() }),
        fetch(`${apiBase}accounting/accounts?active=true`, { headers: authHeaders() }),
      ])
      const bankData = await bankRes.json()
      const accData = await accRes.json()
      setItems(bankData?.data ?? [])
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const url = editingId ? `${apiBase}treasury/bank-accounts/${editingId}` : `${apiBase}treasury/bank-accounts`
      ...
      toast.success(editingId ? "Cuenta bancaria actualizada" : "Cuenta bancaria creada")
  ...
  const handleToggle = async (item: BankAccount) => {
    try {
      const endpoint = item.active ? "deactivate" : "activate"
      const res = await fetch(`${apiBase}treasury/bank-accounts/${item.id}/${endpoint}`, {
        method: "PATCH",
        headers: authHeaders(),
      })
      if (!res.ok) {
      ...
      await fetchAll()
      toast.success(`Cuenta ${item.active ? "desactivada" : "activada"}`)
    } catch {
      toast.error("Error al cambiar el estado")
    }
  }
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Cuentas bancarias de tu empresa</p>
        </div>
        <Button
          onClick={() => {
            setEditingId(null)
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-gray-400 text-sm">No hay cuentas bancarias todavía</div>
      ) : (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {["Banco", "Número", "Tipo", "Estado", "Acciones"].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3 text-sm text-gray-900 whitespace-nowrap">{item.bankName}</td>
                  <td className="px-5 py-3 text-sm font-mono text-gray-700 whitespace-nowrap">{item.accountNumber}</td>
                  <td className="px-5 py-3 text-sm text-gray-500 whitespace-nowrap">{item.accountType}</td>
        ...
        <DialogContent className="bg-white w-[calc(100%-1.5rem)] sm:max-w-md rounded-3xl p-0 border-none shadow-2xl flex flex-col">
          ...
          <div className="px-6 py-6 space-y-4">
            <div className="flex flex-col gap-1.5">
              <Label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest ml-1">Banco</Label>
              <Input value={form.bankName} onChange={(e) => setForm({ ...form, bankName: e.target.value })} className="h-12 rounded-xl border-gray-200" />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest ml-1">Número de cuenta</Label>
              <Input value={form.accountNumber} onChange={(e) => setForm({ ...form, accountNumber: e.target.value })} className="h-12 rounded-xl border-gray-200" />
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              <Label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest ml-1">Cuenta contable asociada (opcional)</Label>
