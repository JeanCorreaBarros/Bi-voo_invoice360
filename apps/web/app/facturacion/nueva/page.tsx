...
export default function NuevaFacturaPage() {
    ...
    useEffect(() => {
        const fetchClients = async () => {
            ...
            try {
                ...
                const apiBase = process.env.NEXT_PUBLIC_API_URL || "https://plasticoslc.com/api/";
                const res = await fetch(apiBase.replace(/\/$/, "") + "/customers", {
                    method: "GET",
                    headers: { "Authorization": `Bearer ${token}` }
                })
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            try {
                const token = getToken();
                const apiBase = process.env.NEXT_PUBLIC_API_URL || "https://plasticoslc.com/api/";
                const res = await fetch(apiBase.replace(/\/$/, "") + "/users?page=1&limit=10", {
                    method: "GET",
                    headers: {
    ...
    useEffect(() => {
        const fetchCompany = async () => {
            try {
                const token = getToken();
                const apiBase = process.env.NEXT_PUBLIC_API_URL || "https://plasticoslc.com/api/";
                const res = await fetch(apiBase.replace(/\/$/, "") + "/companies", {
                    headers: { "Authorization": `Bearer ${token}` }
                });
                if (res.ok) {
                    const data = await res.json();
    ...
    useEffect(() => {
        const fetchResolutions = async () => {
            ...
            const apiBase = process.env.NEXT_PUBLIC_API_URL || "https://plasticoslc.com/api/";
            try {
                const res = await fetch(apiBase.replace(/\/$/, "") + "/resolutions", {
                    headers: {
                        "Authorization": `Bearer ${token}`,
                    },
                });
    ...
    // Genera y descarga un PDF real a partir de la hoja de la simulación
    // (todavía no existe la factura en el backend, así que se arma en el
    // navegador a partir de lo que se está viendo).
    const [isGeneratingDraftPdf, setIsGeneratingDraftPdf] = useState(false)
    const handleDownloadDraftPdf = async () => {
        const element = document.getElementById("invoice-simulation-print")
        if (!element) return

        setIsGeneratingDraftPdf(true)
        try {
            ...
            pdf.save(`borrador-factura-${getOrderPrefix() || "FV"}.pdf`)
        } catch (err) {
            console.error("Error generando PDF del borrador:", err)
            toast.error("No se pudo generar el PDF del borrador")
        } finally {
            setIsGeneratingDraftPdf(false)
        }
    }
    ...
    const handleShowInvoicePreview = async (invoiceId: string | number) => {
        ...
        try {
            const apiBase = process.env.NEXT_PUBLIC_API_URL || "https://plasticoslc.com/api/";
            const res = await fetch(apiBase.replace(/\/$/, "") + `/invoice-documents/${invoiceId}/pdf`, {
                method: 'GET',
                headers: {
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            const apiBase = process.env.NEXT_PUBLIC_API_URL || "https://plasticoslc.com/api/";
            const res = await fetch(apiBase.replace(/\/$/, "") + "/invoices", {
                method: "POST",
                headers: myHeaders,
                ...
                >
                    ...
                    <motion.div variants={itemVariants} className={`grid grid-cols-1 md:grid-cols-2 gap-8 mb-6 ${currentStep === 3 ? "" : "hidden md:grid"}`}>
                        ...
                        <Dialog open={isNewClientOpen} onOpenChange={(open) => setIsNewClientOpen(open)}>
                            <DialogContent className="w-[95vw] bg-white sm:max-w-[540px] max-h-[90vh] overflow-y-auto rounded-2xl p-6 shadow-xl border-0">
                                <DialogHeader className="mb-2">
                                    <DialogTitle className="text-xl font-bold text-gray-900">Crear Nuevo Cliente</DialogTitle>
                                </DialogHeader>
                                ...
                                <div className="grid gap-4 py-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-1 gap-4">
                                        <div className="grid gap-2">
                                            ...
                                            <Input className="bg-[#F8F9FC] border border-gray-200 rounded-xl h-11 text-sm shadow-sm"
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                id="email"
                                                className="bg-[#F8F9FC] border border-gray-200 rounded-xl h-11 text-sm shadow-sm"
                                                type="email"
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="grid gap-2">
                                            ...
                                            <Input id="nit" className="bg-[#F8F9FC] border border-gray-200 rounded-xl h-11 text-sm shadow-sm"
                                                value={newClient.nit} onChange={e => setNewClient({ ...newClient, nit: e.target.value })} />
                                        </div>

                                        <div className="grid gap-2">
                                            <Label htmlFor="address">Dirección</Label>
                                            <Input id="address" className="bg-[#F8F9FC] border border-gray-200 rounded-xl h-11 text-sm shadow-sm"
                                ...
                                <DialogFooter className="flex flex-col sm:flex-row gap-3 mt-6">
                                    <Button variant="outline" onClick={() => { setIsNewClientOpen(false); setNewClient(emptyNewClient); }} className="w-full sm:w-auto h-11 px-8 rounded-xl font-semibold border-gray-200 text-gray-700 hover:bg-gray-50">Cancelar</Button>
                                    <Button onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            if (!res.ok) {
                                                ...
                                                toast.error("Error creando cliente. Revisa la consola.")
                                                return
                                            }
                                    ...
                                    }} className="w-full sm:w-auto h-11 px-8 rounded-xl font-semibold bg-[hsl(209,79%,27%)] hover:bg-[hsl(209,79%,20%)] text-white shadow-md">Guardar</Button>
                                </DialogFooter>
                            </DialogContent>
                        </Dialog>
                    </motion.div>
                ...
                <Dialog open={showSimulationModal} onOpenChange={setShowSimulationModal}>
                    <DialogContent className="max-w-4xl w-[95vw] h-[90vh] bg-gray-100 p-0 overflow-y-auto z-[120]">
                        ...
                        <div className="p-4 md:p-8 flex justify-center">
                            <div id="invoice-simulation-print" className="relative isolate bg-white w-full max-w-[800px] min-h-[1000px] shadow-2xl overflow-hidden flex flex-col font-sans text-gray-800">

                                {/* Marca de agua: igual que el PDF real (logo grande, tenue, centrado) */}
                                <img
                                    src={logoPreview || "/invoices360-logo.png"}
                                    alt=""
                                    aria-hidden="true"
                                    className="pointer-events-none select-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[560px] opacity-[0.14] grayscale z-0"
                                />

                                {/* Header — igual que el PDF (barra azul, "FACTURA DE VENTA" en blanco) */}
                                <div className="relative z-10 bg-[hsl(209,79%,27%)] px-8 md:px-12 py-5">
                                    <p className="text-white text-lg md:text-xl font-black tracking-wide">FACTURA DE VENTA</p>
                                </div>

                                <div className="relative z-10 flex flex-col flex-1 p-8 md:p-12 pt-8">
                                    {/* Header - 3 Equal Sections */}
                                    <div className="grid grid-cols-1 md:grid-cols-3 items-start gap-8 mb-10 border-b border-gray-200 pb-8">
                                        {/* Column 1: Logo */}
                                        <div className="flex justify-start">
                                            <div className="w-28 h-28 bg-white flex items-center justify-center p-2 rounded-xl border border-gray-100 shadow-sm">
                                                {logoPreview ? (
                                                    ...
                                                    <img src="/invoices360-logo.png" alt="Invoices360" className="w-full h-full object-contain" />
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-col items-center text-center">
                                            <h1 className="text-lg font-black uppercase text-gray-900 tracking-tight mb-1 leading-tight">
                                                {company?.tradeName || company?.businessName || company?.name || "Empresa No Configurada"}
                                            </h1>
                                            <div className="space-y-0">
                                                <p className="text-[11px] font-bold text-gray-800">NIT: {company?.documentNumber || company?.nit || "000000000"}</p>
                                                <p className="text-[10px] text-gray-600 font-bold uppercase tracking-tight">Email: {company?.email || "correo@empresa.com"}</p>
                                                <p className="text-[10px] text-gray-600 font-bold uppercase tracking-tight">Teléfono: {company?.cellphone || company?.phone || "0000000000"}</p>
                                                <p className="text-[10px] text-gray-600 font-bold uppercase tracking-tight">Dirección: {company?.address || "Dirección no especificada"}</p>
                                                <p className="text-[10px] text-gray-900 font-black uppercase tracking-widest mt-1 border-t border-gray-100 pt-1">Resolución: 1</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex justify-end">
                                            <div className="text-right space-y-1">
                                                <div className="flex justify-between items-center gap-3">
                                                    ...
                                                    <span className="text-xs font-black text-gray-900">{getOrderPrefix()}</span>
                                                </div>
                                                <div className="flex justify-between items-center gap-3">
                                                    <span className="text-[10px] font-black uppercase text-gray-400">Número:</span>
                                                    <span className="text-xs font-black text-gray-900">26</span>
                                                </div>
                                    ...
                                    {/* Customer Box — igual que el PDF: caja con borde, sin relleno */}
                                    <div className="border border-gray-300 rounded-md p-6 mb-8">
                                        <h3 className="text-xs font-black uppercase tracking-wide text-gray-900 mb-4">Datos del Cliente</h3>
                                        ...
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="bg-gray-100">
                                                    <th className="py-3 px-4 text-[10px] font-black uppercase tracking-wider text-gray-600">Descripción</th>
                                                    <th className="py-3 px-4 text-[10px] font-black uppercase tracking-wider text-gray-600 text-center">Cant</th>
                                                    <th className="py-3 px-4 text-[10px] font-black uppercase tracking-wider text-gray-600 text-right">Precio</th>
                                                    <th className="py-3 px-4 text-[10px] font-black uppercase tracking-wider text-gray-600 text-right">Desc</th>
                                                    <th className="py-3 px-4 text-[10px] font-black uppercase tracking-wider text-gray-600 text-right">IVA</th>
                                                    <th className="py-3 px-4 text-[10px] font-black uppercase tracking-wider text-gray-600 text-right">Total</th>
                                                </tr>
                                            </thead>
                                                ...
                                                {items.map((item, idx) => {
                                                    ...
                                                    return (
                                                        <tr key={item.id} className="text-sm">
                                                            ...
                                                            <td className="py-4 px-4 text-right font-black text-gray-900">{formatCurrency(totalItem).replace("$", "")}</td>
                                                        </tr>
                                                    );
                                                })}
                                            </tbody>
                                    ...
                                    {/* Totals Section — caja con borde igual al PDF */}
                                    <div className="mt-10 flex justify-end">
                                        <div className="w-full max-w-[320px] space-y-2 border border-gray-300 rounded-md p-6">
                                    ...
                                    {/* Footer info — mismo texto que el PDF real */}
                                    <div className="mt-auto pt-8 text-[10px] text-gray-400 font-medium flex justify-between items-end">
                                        <p>Representación gráfica de la factura electrónica.</p>
                                        <p>Firmado digitalmente por el sistema ERP</p>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="sticky bottom-0 z-20 bg-white border-t p-4 flex justify-center gap-4">
                            <Button onClick={handleDownloadDraftPdf} disabled={isGeneratingDraftPdf} variant="outline" className="gap-2">
                                <Printer size={16} /> {isGeneratingDraftPdf ? "Generando PDF..." : "Imprimir borrador"}
                            </Button>
